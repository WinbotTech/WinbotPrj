#ifndef _SELFCANDIDATE_HPP_
#define _SELFCANDIDATE_HPP_

/*header*/

/**
 * Generated with Acceleo
 */
//package RexMind;

#include "..//Resources/PrimitiveTypes.hpp"
#include "..//Resources/DataTypes.hpp"
#include "..//Resources/Enumerations.hpp"
#include "..//Resources/vectorTemplate.hpp"
#include "..//Resources/TerminalIO.hpp"
// Start of user code Provide additional headers here for class SelfCandidate.
// End of user code

/**
 * \authors Daniel Kaminski de Souza
 * \authors Marcelo Giesteira Campani
 */
/** 
 * Forward class declarations
 */

class SelfCandidate 
{
// Start of user code Provide additional header members here for class SelfCandidate.
// End of user code
};

#endif //_SELFCANDIDATE_HPP_
